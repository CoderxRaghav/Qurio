    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 relative">
      ...
      >
        <div className="glass-panel rounded-2xl p-8 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.div className="space-y-1.5" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              ...
              <div className="relative">
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="input-glass w-full pl-10 pr-4 py-3 rounded-xl text-sm" placeholder="John Doe" />
              </div>
            </motion.div>
            ...
            <motion.div className="space-y-1.5" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 }}>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="input-glass w-full pl-10 pr-4 py-3 rounded-xl text-sm" placeholder="you@example.com" />
              </div>
            </motion.div>
            ...
            <motion.div className="space-y-1.5" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} className="input-glass w-full pl-10 pr-12 py-3 rounded-xl text-sm" placeholder="••••••••" />
            ...
            <motion.div className="space-y-1.5" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 }}>
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} className="input-glass w-full pl-10 pr-10 py-3 rounded-xl text-sm" placeholder="••••••••" />
                {confirm.length > 0 && (
          ...
          <motion.button className="btn-glass w-full py-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2" whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}>
